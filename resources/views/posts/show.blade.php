                ...
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{ route('posts.destroy', $post) }}" method="POST">
                        ...
                        <input 
                            ...
                            class="bg-red-500 hover:bg-red-700 p-2 text-white font-bold mt-4 cursor-pointer"
                        >
                    </form>
                @endif
            @endauth
                ...
                @auth
                    ...
                    <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $post->user]) }}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            @error('comentario')
                                <span class="text-white bg-red-500 block rounded-lg text-center text-sm mt-1 p-1">{{ $message }}</span>
                            @enderror
                        </div>
